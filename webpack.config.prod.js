...
var ossConfig = config.get('OSSServer');
...
var staticBasePath = ossConfig.staticBasePath + dateFormat((new Date()), 'yyyymmddHHMMss') + '_' + uid.sync(18) + '/';
var publicPath = ossConfig.address + '/' + staticBasePath;
...
module.exports = {
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      filename: 'index.html',
      template: './src/public/stable.html',
      inject: true, // 自动注入
      minify: {
    ...
    new CopyWebpackPlugin([{
      from: './src/public'
        ...
        // By default, we only copy modified files duringd
    ...
    new AppCachePlugin({
      ...
      // exclude: [/.*\.html$/, /.*\.css$/, /.*\.js$/],
      output: 'local.appcache'
    }),
    new AliOssPlugin({
      accessKeyId: ossConfig.accessKeyId,
      accessKeySecret: ossConfig.accessKeySecret,
      region: ossConfig.region, // eg: oss-cn-hangzhou
      bucket: ossConfig.bucket,
      prefix: staticBasePath, // oss目录前缀; eg: auto_upload_ci/test
      exclude: /.*\.html$/,   // 选填, 默认: /.*/
